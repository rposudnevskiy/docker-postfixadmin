...
init_or_upgrade_db() {
  ...
  if [ $1 = "init" ]; then
    ...
    echo "insert into admin (username,password,superadmin) values ('$ADMIN_USERNAME','$ADMIN_PASSWORD',1)" | mysql --host=$MYSQL_HOST --user=$MYSQL_USER --password=$MYSQL_PASSWORD $MYSQL_DATABASE &>/dev/null
  else
